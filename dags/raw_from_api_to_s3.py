...
def debug_context(**context):
    logging.info("--- ПОЛНЫЙ СПИСОК КОНТЕКСТА AIRFLOW ---")
    # Красиво печатаем весь словарь
    logging.info(pprint.pformat(context))
...
with DAG(
    ...
    debug_task=PythonOperator(
        task_id="debug_airflow_context",
        python_callable=debug_context,
    )
    ...
    start >> debug_task >> get_and_transfer_api_data_to_s3 >> end